    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white">
      ...
      <header className="border-b border-white/10 bg-[#0a0a0f]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="/api/briefing" target="_blank" className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white/60 hover:bg-white/10 transition">📬 Send Briefing</a>
            <button onClick={logout} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white/60 hover:bg-white/10 transition">Logout</button>
          </div>
        </div>
      </header>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                <h2 className="font-bold text-white mb-4">Pipeline Snapshot</h2>
                <div className="flex gap-2 flex-wrap">
                  {stageCounts.map(s => (
                    <div key={s.stage} className="flex-1 min-w-[80px] bg-white/5 rounded-xl p-3 text-center">
                      <div className="text-xs text-white/40 uppercase tracking-wider">{s.stage}</div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                <h2 className="font-bold text-white mb-4">🔴 Urgent Tasks</h2>
                {urgentTasks.length === 0 ? (
                  <p className="text-white/30 text-sm">No urgent tasks. You&apos;re on track! ✅</p>
                ) : (
                  <div className="space-y-2">
                    {urgentTasks.slice(0, 5).map(t => (
                      <div key={t.id} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
                        <button onClick={() => toggleTask(t.id, t.status)} className="w-5 h-5 rounded-full border-2 border-red-500/50 flex-shrink-0 hover:bg-red-500/20 transition" />
            ...
            {/* Recent Activity */}
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
              <h2 className="font-bold text-white mb-4">Recent Rep Activity</h2>
              {activities.length === 0 ? (
                <p className="text-white/30 text-sm">No activities yet. Activities appear when reps start logging calls and demos.</p>
              ) : (
                <div className="space-y-2">
                    ...
                    return (
                      <div key={a.id} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="font-medium text-white/90 text-sm">{rep?.name || "Unknown"}</span>
                            <span className="text-white/40 text-sm ml-2">{a.type} {a.notes ? `— ${a.notes}` : ""}</span>
                          </div>
                        </div>
                        <span className="text-xs text-white/30">{new Date(a.created_at).toLocaleString()}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {stages.map(stage => {
                ...
                return (
                  <div key={stage} className={`rounded-2xl border ${colors[stage] || "border-white/10"} bg-white/[0.02] p-4`}>
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-xs font-bold uppercase text-white/50 tracking-wider">{stage}</h3>
                      <span className="bg-white/10 text-white/60 text-xs font-bold px-2 py-0.5 rounded-full">{stageLeads.length}</span>
                    </div>
                    <div className="space-y-2">
          ...
          <div className="space-y-6">
            ...
            {reps.length === 0 ? (
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-16 text-center">
                ...
                <p className="text-white/40 text-lg">No sales reps yet</p>
                <p className="text-white/25 text-sm mt-1">Add your first rep to start tracking performance</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {reps.map(rep => {
                  ...
                  return (
                    <div key={rep.id} className="rounded-2xl border border-white/10 bg-white/[0.02] p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="font-bold text-white">{rep.name}</h3>
                          <p className="text-xs text-white/40">{rep.email}</p>
                        </div>
                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${rep.status === "active" ? "bg-green-500/20 text-green-400" : "bg-white/10 text-white/40"}`}>{rep.status}</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="bg-white/5 rounded-xl p-2"><div className="text-lg font-bold text-white">{repLeads.length}</div><div className="text-xs text-white/40">Leads</div></div>
                        <div className="bg-white/5 rounded-xl p-2"><div className="text-lg font-bold text-white">{repActs.length}</div><div className="text-xs text-white/40">Activities</div></div>
                        <div className="bg-white/5 rounded-xl p-2"><div className="text-lg font-bold text-white">{repActs.filter(a => a.type === "demo").length}</div><div className="text-xs text-white/40">Demos</div></div>
                      </div>
                    </div>
                  );
                })}
        ...
        {tab === "tasks" && (
          <div className="space-y-6">
            ...
            {taskFilter !== "done" && urgentTasks.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-red-400 uppercase tracking-wider mb-3">🔴 Urgent</h3>
                <div className="space-y-2">
                  {urgentTasks.map(t => (
                    <div key={t.id} className="flex items-center gap-3 bg-red-500/5 border border-red-500/10 rounded-xl p-4">
                      <button onClick={() => toggleTask(t.id, t.status)} className="w-6 h-6 rounded-full border-2 border-red-500/50 flex-shrink-0 hover:bg-red-500/20 transition flex items-center justify-center" />
                      <div className="flex-1"><p className="text-sm text-white/90">{t.title}</p>{t.description && <p className="text-xs text-white/40 mt-1">{t.description}</p>}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {taskFilter !== "done" && highTasks.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider mb-3">🟠 High Priority</h3>
                <div className="space-y-2">
                  {highTasks.map(t => (
                    <div key={t.id} className="flex items-center gap-3 bg-white/[0.02] border border-white/10 rounded-xl p-4">
                      <button onClick={() => toggleTask(t.id, t.status)} className="w-6 h-6 rounded-full border-2 border-amber-500/50 flex-shrink-0 hover:bg-amber-500/20 transition flex items-center justify-center" />
                      <div className="flex-1"><p className="text-sm text-white/90">{t.title}</p>{t.description && <p className="text-xs text-white/40 mt-1">{t.description}</p>}</div>
                    </div>
                  ))}
                </div>
            ...
            {taskFilter !== "done" && medTasks.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-yellow-400 uppercase tracking-wider mb-3">🟡 Medium</h3>
                <div className="space-y-2">
                  {medTasks.map(t => (
                    <div key={t.id} className="flex items-center gap-3 bg-white/[0.02] border border-white/10 rounded-xl p-4">
                      <button onClick={() => toggleTask(t.id, t.status)} className="w-6 h-6 rounded-full border-2 border-white/20 flex-shrink-0 hover:bg-white/10 transition flex items-center justify-center" />
            ...
            {taskFilter !== "done" && lowTasks.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-green-400 uppercase tracking-wider mb-3">🟢 Low</h3>
                <div className="space-y-2">
                  {lowTasks.map(t => (
                    <div key={t.id} className="flex items-center gap-3 bg-white/[0.02] border border-white/10 rounded-xl p-4">
                      <button onClick={() => toggleTask(t.id, t.status)} className="w-6 h-6 rounded-full border-2 border-white/10 flex-shrink-0 hover:bg-white/10 transition flex items-center justify-center" />
            ...
            {(taskFilter === "all" || taskFilter === "done") && doneTasks.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-white/30 uppercase tracking-wider mb-3">✅ Completed ({doneTasks.length})</h3>
                <div className="space-y-2">
                  {doneTasks.map(t => (
